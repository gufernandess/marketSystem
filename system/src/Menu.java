...
/**
 * Aqui instanciamos(de forma estática) todas as listas que são acessadas durante a execução do programa.
 * 
 * Basicamente há os métodos de login, cadastro e dashboard. A navegação do sistema passa por eles.
 * 
 * A navegação é feita através do input de inteiros, visto que assim há menos casos de erro.
 * 
 */
...
public class Menu {
    ...
    /**
     * O login é a tela inicial do sistema, onde é possível entrar na dashboard ou cadastrar uma conta.
     */
    ...
    if(response == 2) register();

    if(response == 3) System.exit(0);
    ...
    /**
     * Aqui é onde as funcionalidades do sistema aparecem, onde é possível fazer toda a gestão do mercantil.
     */
